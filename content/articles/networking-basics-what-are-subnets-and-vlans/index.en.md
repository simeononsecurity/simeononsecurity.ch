...
A **subnet** is a logical subdivision of an IP network. It allows the network administrator to divide a large network into smaller, more manageable segments. Each subnet has its own unique IP address range, which helps in efficient use of available IP addresses.
...
3. **Efficient Resource use:** Subnetting allows for better allocation and use of IP addresses, which are a limited resource in the IPv4 addressing scheme.
