---
title: "Атака на цепочку поставок SolarWinds Orion: C2, смягчение последствий и рекомендации экспертов"
date: 2020-12-14
toc: true
draft: false
description: "Узнайте об атаке на цепочку поставок SolarWinds Orion, в том числе о вредоносном ПО C2, мерах по смягчению последствий и рекомендациях экспертов от таких организаций, как DHS и FireEye."
tags: ["Солнечные ветры", "Цепочка поставок", "С2", "Командование и контроль", "Смягчения", "Руководство", "Разведка угроз", "МДС", "Огненный глаз", "МСЦ", "Майкрософт", "САНС", "SolarWinds.Orion.Core.BusinessLayer.dll", "SolarWinds.BusinessLayerHost.exe", "ConfigurationWizard.exe", "NetflowDatabaseMaintenance.exe", "NetFlowService.exe", "SolarWinds.Administration.exe", "SolarWinds.Коллектор.Service.exe", "DNS CNAME", "Диапазоны IP-адресов", "DNS-имена"]
---

Руководство по проблеме солнечного ветра см.[DHS](https://cyber.dhs.gov/ed/21-01/), [SolarWinds](https://www.solarwinds.com/securityadvisory), [FireEYE](https://www.fireeye.com/blog/threat-research/2020/12/evasive-attacker-leverages-solarwinds-supply-chain-compromises-with-sunburst-backdoor.html), [MSRC](https://msrc-blog.microsoft.com/2020/12/13/customer-guidance-on-recent-nation-state-cyber-attacks/), and [Microsoft](https://www.microsoft.com/en-us/wdsi/threats/malware-encyclopedia-description?Name=Trojan:MSIL/Solorigate.B!dha) 

Опытные пользователи, см.[FireEYE Countermeasures Repo](https://github.com/fireeye/sunburst_countermeasures) по этому вопросу.

У SANS есть хорошее видео на эту тему[here](https://www.youtube.com/watch?v=4tmlZCk2gCg&feature=youtu.be)

## Исполняемые файлы:
Рассматриваемая DLL```SolarWinds.Orion.Core.BusinessLayer.dll``` and was signed as a ligitimate part of the SolarWinds suite, bypassing application control technologies. It is installed as a service.
